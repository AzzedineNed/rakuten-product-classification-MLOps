...
No MLflow code lives here. Publishing a saved model as a registry version and
pulling one back are rakuten_img.tracking.register_model / .load_from_registry,
alongside the rest of this modality's MLflow surface. The registered artifact is
the SAME joblib payload this module writes, so serving code handles a registry
load and a local load identically.
"""
